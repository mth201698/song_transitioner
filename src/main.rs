...
const FADE_DURATION: usize = 44100 * 5; // 5 seconds fade at 44.1kHz
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Align beats (for simplicity, using the first peak as a starting point)
    let beat1 = find_first_peak(&samples1);
    let beat2 = find_first_peak(&samples2);
    ...
    for &sample in &samples1[..beat1] {
        output.write_sample(sample[0])?;
        output.write_sample(sample[1])?;
    }
    ...
    for i in 0..FADE_DURATION {
        ...
        let sample1 = samples1[beat1 + i].scale_amp(fade_out);
        let sample2 = adjusted_samples2[beat2 + i].scale_amp(fade_in);
        let mixed = [sample1[0] + sample2[0], sample1[1] + sample2[1]];

        output.write_sample(mixed[0])?;
        output.write_sample(mixed[1])?;
    }

    // Write the remaining part of song2
    for &sample in &adjusted_samples2[beat2 + FADE_DURATION..] {
        output.write_sample(sample[0])?;
        output.write_sample(sample[1])?;
    }
